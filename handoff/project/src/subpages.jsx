...
function AboutPage({ navigate }) {
  return (
    <div className="page active" data-screen-label="04 О нас">
      <section className="about-hero">
        <div className="wrap">
          ...
          <p className="about-lede">
            Мы верим, что напечатанное — <em>живёт дольше</em>, чем написанное. С 2006 года мы печатаем книги, каталоги и упаковку, которые не хочется выбрасывать.
          </p>
        </div>
      </section>

      <section className="about-body">
        <div className="wrap" style={{ display: 'contents' }}></div>
        <div className="wrap" style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 64 }}>
          <div>
            <div className="label">Философия</div>
          </div>
          <div>
            <p>Типография «Сити Принт» — производство полного цикла в Екатеринбурге. На нашем счету более пяти тысяч заказов: от тиражей в пятьдесят экземпляров до корпоративных книг для федеральных компаний.</p>
            <p>Мы не гонимся за самой низкой ценой — гонимся за долговечным результатом. Поэтому наши клиенты возвращаются: семьдесят процентов заказов — от тех, кто уже работал с нами.</p>
            <p>Здесь живут офсетные и цифровые машины, переплётный цех, линия премиум-отделки: тиснение фольгой, конгрев, шелкография, вырубка. Всё под одной крышей — значит, мы отвечаем за качество на каждом этапе.</p>
          </div>
        </div>
      </section>
        ...
        <div className="wrap">
          <div className="section-head">
            <div>
              ...
              <h2>33 человека, одна типография</h2>
            </div>
            <p className="sub">Технологи, верстальщики, печатники, переплётчики, менеджеры. Средний стаж в компании — 7 лет.</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }} className="team-grid">
            {[
              {n: 'Ирина Новикова',   r: 'Главный технолог',    img: UNSPLASH('1573496359142-b8d87734a5a2', 600)},
              {n: 'Сергей Белов',     r: 'Начальник цеха',      img: UNSPLASH('1472099645785-5658abf4ff4e', 600)},
              {n: 'Анна Петрова',     r: 'Менеджер проектов',   img: UNSPLASH('1580489944761-15a19d654956', 600)},
              {n: 'Пётр Суворов',     r: 'Печатник',            img: UNSPLASH('1507003211169-0a1dd7228f2d', 600)},
              {n: 'Марина Ч.',        r: 'Переплётчик',         img: UNSPLASH('1534528741775-53994a69daeb', 600)},
              {n: 'Виктор Козлов',    r: 'Препресс',            img: UNSPLASH('1519085360753-af0119f7cbe7', 600)},
              {n: 'Елена Крылова',    r: 'Менеджер по работе с клиентами', img: UNSPLASH('1544005313-94ddf0286df2', 600)},
              {n: 'Дмитрий Соколов',  r: 'Директор',            img: UNSPLASH('1560250097-0b93528c311a', 600)},
            ].map((p, i) => (
              <div key={i}>
                <Placeholder src={p.img} label={p.n} style={{ aspectRatio: '3/4', borderRadius: 14 }} />
                <div style={{ marginTop: 12, fontFamily: 'var(--font-display)', fontSize: 17, color: '#fff' }}>{p.n}</div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.08em', textTransform: 'uppercase', marginTop: 2 }}>{p.r}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <style>{`@media (max-width: 860px){ .team-grid { grid-template-columns: 1fr 1fr !important; } }`}</style>
    </div>
  );
}
