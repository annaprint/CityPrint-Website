...
// Block 3.5: Unique info block (tables, lists, guides per landing)
function LandingUniqueBlock({ data }) {
  const ref = useReveal();

  // Tiers table (catalogs: Эконом/Стандарт/Премиум)
  if (data.tiers) {
    return (
      <section className="landing-unique reveal" ref={ref}>
        <div className="wrap">
          <h3 className="landing-h3">Эконом / Стандарт / Премиум</h3>
          <div className="info-table">
            <div className="info-table-header">
              <span>Вариант</span><span>Для чего подходит</span><span>Что внутри</span>
            </div>
            {data.tiers.map((row, i) => (
              <div key={i} className="info-table-row">
                <span className="info-table-accent">{row.tier}</span>
                <span>{row.forWhat}</span>
  ...
  // Audience table (books: Для кого печатаем)
  if (data.audienceTable) {
    return (
      <section className="landing-unique reveal" ref={ref}>
        <div className="wrap">
          <h3 className="landing-h3">Для кого печатаем</h3>
          <div className="info-table">
            <div className="info-table-header">
              <span>Для кого</span><span>Что важно</span><span>Что предложим</span>
            </div>
            {data.audienceTable.map((row, i) => (
              <div key={i} className="info-table-row">
                <span className="info-table-accent">{row.audience}</span>
  ...
  // Material guide (stickers: Как выбрать материал)
  if (data.materialGuide) {
    return (
      <section className="landing-unique reveal" ref={ref}>
        <div className="wrap">
          <h3 className="landing-h3">Как выбрать материал</h3>
          <div className="info-table">
            <div className="info-table-header">
              <span>Материал</span><span>Подходит</span><span>Не подходит</span>
            </div>
            {data.materialGuide.map((row, i) => (
              <div key={i} className="info-table-row">
                <span className="info-table-accent">{row.material}</span>
                <span>{row.fits}</span>
                <span>{row.notFits}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  // Season guide (calendars: Когда заказывать)
  if (data.seasonGuide) {
    return (
      <section className="landing-unique reveal" ref={ref}>
        <div className="wrap">
          <h3 className="landing-h3">Когда заказывать</h3>
          <div className="info-table info-table--two-col">
            <div className="info-table-header">
              <span>Период</span><span>Что происходит</span>
            </div>
            {data.seasonGuide.map((row, i) => (
              <div key={i} className="info-table-row">
                <span className="info-table-accent">{row.period}</span>
                <span>{row.note}</span>
              </div>
            ))}
          </div>
  ...
  // Personalizations list (souvenirs: Что можно персонализировать)
  if (data.personalizations) {
    return (
      <section className="landing-unique reveal" ref={ref}>
        <div className="wrap">
          <h3 className="landing-h3">Что можно персонализировать</h3>
          <div className="personalization-grid">
            {data.personalizations.map((item, i) => (
              <div key={i} className="personalization-item">{item}</div>
            ))}
          </div>
        </div>
      </section>
  ...
  // Kit contents (presentations: Что входит в комплект)
  if (data.kitContents) {
    return (
      <section className="landing-unique reveal" ref={ref}>
        <div className="wrap">
          <h3 className="landing-h3">Что входит в презентационный комплект</h3>
          <div className="personalization-grid">
            {data.kitContents.map((item, i) => (
              <div key={i} className="personalization-item">{item}</div>
            ))}
          </div>
        </div>
  ...
  // Event kits (events: Комплект под формат мероприятия)
  if (data.eventKits) {
    return (
      <section className="landing-unique reveal" ref={ref}>
        <div className="wrap">
          <h3 className="landing-h3">Комплект под формат мероприятия</h3>
          <div className="info-table info-table--two-col">
            <div className="info-table-header">
              <span>Формат</span><span>Что может понадобиться</span>
            </div>
            {data.eventKits.map((row, i) => (
              <div key={i} className="info-table-row">
                <span className="info-table-accent">{row.eventType}</span>
                <span>{row.items}</span>
              </div>
            ))}
          </div>
  ...
  // Selection guide (bags: Какой пакет выбрать)
  if (data.selectionGuide) {
    return (
      <section className="landing-unique reveal" ref={ref}>
        <div className="wrap">
          <h3 className="landing-h3">Какой пакет выбрать</h3>
          <div className="info-table info-table--two-col">
            <div className="info-table-header">
              <span>Вид пакета</span><span>Для чего подходит</span>
            </div>
            {data.selectionGuide.map((row, i) => (
              <div key={i} className="info-table-row">
                <span className="info-table-accent">{row.type}</span>
                <span>{row.fits}</span>
              </div>
            ))}
          </div>
...
function LandingCTA({ title, text }) {
  const [form, setForm] = useStateL({ name: '', phone: '', task: '', consent: false });
  const [sent, setSent] = useStateL(false);
  const update = (k, v) => setForm({ ...form, [k]: v });
  const canSubmit = form.name && form.phone && form.consent;
      ...
      <div className="wrap">
        <div className="landing-cta-grid">
          ...
          <div className="landing-cta-form">
            ...
            <button className="btn primary" style={{ width: '100%', justifyContent: 'center' }} onClick={() => { if (canSubmit) setSent(true); }} disabled={!canSubmit}>Отправить заявку</button>
          </div>
        </div>
      </div>
    </section>
...
Object.assign(window, {
  LandingHero, LandingScenarios, LandingVariants, LandingUniqueBlock, LandingWhyMatters,
