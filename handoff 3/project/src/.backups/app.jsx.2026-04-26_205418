...
function CookieBanner() {
  const [visible, setVisible] = useStateA(() => !localStorage.getItem('citi-cookies-accepted'));
  if (!visible) return null;
  const accept = () => { localStorage.setItem('citi-cookies-accepted', '1'); setVisible(false); };
  return (
    <div className="cookie-banner">
      <div className="cookie-banner-content">
        <p>Мы используем файлы cookie для аналитики и улучшения работы сайта. Продолжая пользоваться сайтом, вы соглашаетесь с <a href="#privacy">политикой использования cookie</a>.</p>
        <button className="btn primary" onClick={accept}>Принять</button>
      </div>
    </div>
  );
}
